...
import {StyleSheet, Text, TouchableOpacity, View, Image, Share,
} from "react-native";
...
import {Card, CardTitle, CardContent, CardAction, CardButton, CardImage, button,
...
export default function Counter(props) {
  ...
  const _subscribe = () => {
    ...
    steps.current = [];
    setSubscription(
      //we set this state variable so later we can use it to unsubscribe
  ...
  const tallyLatestSteps = async () => {
    ...
    if (recentAccelerationData.current.length > 0) {
      ...
      const { spikes, previousHighPointTime } = getSpikesFromAccelerometer({
      ...
      previousValue.current =
        recentAccelerationData.current[
          recentAccelerationData.current.length - 1
        ].value; //store this for when we need to remember the last value
  ...
  if (currentScreen === "counter") {
    return (
      <View style={styles.screen}>
        ...
        >
          <CardTitle subtitle={"Steps"} title={stepCount} />
          <FontAwesome5
            name="redo"
            color="red"
            size={20}
        ...
        >
          <TouchableOpacity onPress={shareProgress} style={{ zIndex: 1 }}>
            <FontAwesome5
              ...
              color="#B4B4B4"
              size={20}
              style={{ marginTop: 20, paddingLeft: 280, position: "absolute" }}
            />
          </TouchableOpacity>
          <CardContent style={{ marginTop: 60, marginLeft: 20 }}>
            <Speedometer width={250} value={score} max={100} min={-100}>
            ...
            />
            <TouchableOpacity onPress={close} style={styles.button2}>
              <Text>close</Text>
            </TouchableOpacity>
            <Text
              style={{ textAlign: "center", color: "#0000EE" }}
